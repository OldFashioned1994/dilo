<p align="center">
  <img src="marca/logo.svg" alt="¡DILO" width="420">
</p>
...
DILO es la respuesta de este lado del mapa:
...
3. **Soltá.** DILO procesa tu voz en tu propia máquina con Whisper o Parakeet.
...
➡️ **[Descargá la última versión](https://github.com/OldFashioned1994/dilo/releases/latest)** — instaladores para Windows (x64), macOS (Intel y Apple Silicon) y Linux (x64).
...
DILO se puede controlar desde la terminal (los flags van a la instancia ya abierta):

```bash
dilo --toggle-transcription    # Grabar / frenar
dilo --toggle-post-process     # Grabar con post-procesado
dilo --cancel                  # Cancelar la operación en curso
dilo --start-hidden            # Arrancar sin mostrar la ventana
dilo --no-tray                 # Arrancar sin ícono de bandeja
dilo --debug                   # Modo debug con logs verbosos
```

En Linux/Wayland podés atar esos comandos a atajos de tu compositor (GNOME, KDE, Sway, Hyprland) o usar señales Unix: `SIGUSR2` alterna la transcripción y `SIGUSR1` la transcripción con post-procesado (`pkill -USR2 -n dilo`).

## Instalación manual de modelos

Si tu red bloquea la descarga automática, bajá los modelos a mano y ponelos en la carpeta `models` del directorio de datos (lo ves en **Ajustes → Acerca de**; típico en Windows: `%APPDATA%\com.dilo.app\models`).
...
- **Linux:** para que el pegado funcione instalá la herramienta de tu display server: `xdotool` (X11), `wtype` (Wayland) o `dotool`. El overlay de grabación viene desactivado por defecto. Si la app no arranca, revisá que esté instalado `gtk-layer-shell` (`libgtk-layer-shell0` en Debian/Ubuntu) o probá `WEBKIT_DISABLE_DMABUF_RENDERER=1 dilo`.

¿Problemas? Abrí un [issue](https://github.com/OldFashioned1994/dilo/issues) — en español o inglés, como te salga.

## Desarrollo

```bash
git clone https://github.com/OldFashioned1994/dilo
cd dilo
bun install
bun tauri dev
```
...
Código bajo [licencia MIT](LICENSE). El nombre DILO, el isotipo «¡» y los íconos son marca propia y no forman parte de la licencia: los forks deben usar su propia identidad, igual que hicimos nosotros.
